...
const comments = client.db(process.env.MONGODB_DB).collection(process.env.MONGODB_COLLECTION);
let goal = Number(process.env.GOAL || 100);
if (!Number.isSafeInteger(goal) || goal < 1) throw new Error('GOAL deve ser um inteiro positivo.');
const files = {
  '/': ['index.html', 'text/html; charset=utf-8'],
  '/style.css': ['style.css', 'text/css; charset=utf-8'],
  '/app.js': ['app.js', 'text/javascript; charset=utf-8'],
  '/fonts/exo-2-latin-wght-normal.woff2': ['fonts/exo-2-latin-wght-normal.woff2', 'font/woff2']
};
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === '/api/comments') {
    ...
    try {
      ...
      const [result] = await comments.aggregate([
        { $match: { active: true, comment: { $type: 'string', $regex: '\\S' }, createdAt: { $gte: start, $lt: end } } },
        { $group: { _id: null, total: { $sum: 1 }, latestCommentAt: { $max: '$createdAt' } } }
      ], { maxTimeMS: 5000 }).toArray();
      res.end(JSON.stringify({ day: start.toISOString(), total: result?.total ?? 0, latestCommentAt: result?.latestCommentAt ?? null, goal }));
    } catch {
      res.writeHead(503).end(JSON.stringify({ error: 'Não foi possível consultar os comentários.' }));
    }
    return;
  }
  ...
  try {
    ...
    res.end(content);
  } catch { res.writeHead(500).end('Não foi possível carregar o painel.'); }
});
